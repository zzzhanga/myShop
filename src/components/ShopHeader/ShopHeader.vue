<template>
  <div class="shop-header">
    ...
    @click="toggleSupportShow"    v-if='info.supports' >
      <div class="discounts-left">
          <!--  vue 解析数据  首先 先解析原始数据   info ={}  
          info.description 不存在   undefined  不会报错
          如果需要动态接下一个三层 数据  需要用 v-if   选判断如果没有 那么不加载dom 
          那么就会报错    等到  info 异步 拿到数据后再来加载 dom   
             info    -->
